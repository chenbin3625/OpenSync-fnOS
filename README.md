...
- **文件过滤** — 文件名、文件后缀、文件目录过滤 + 文件大小范围限制
- **实时任务** — 查看当前任务快照，按状态和分页定期刷新逐文件明细，慢连接丢帧自动重同步，支持失败后手动重试
- **执行历史** — 分页查看历史任务，按状态 / 类型 / 关键词筛选
- **多渠道通知** — Webhook、Server酱、钉钉、企业微信、飞书；异步投递不阻塞任务完成，界面显示最近投递状态
- **系统设置** — 并发数、重试次数、超时时间、历史记录保留天数
- **多语言支持** — 内置中文和英文界面消息，可通过配置文件切换语言
...
3. **文件过滤** — 按需设置文件大小限制，以及文件名、文件后缀和文件目录过滤

文件过滤只支持以下三类规则：

- **文件名**：匹配文件名，可使用简单的 `*` 通配符，例如 `Thumbs.db`、`._*`
- **文件后缀**：使用 `*.后缀`，例如 `*.tmp`、`*.pdf`，不区分后缀大小写
- **文件目录**：以 `/` 结尾，例如 `cache/`、`photos/raw/`；单级目录名可匹配任意层级

其他 Gitignore 语法（包括 `!` 反选、字符集和正则表达式）不受支持，保存时会明确提示拒绝。

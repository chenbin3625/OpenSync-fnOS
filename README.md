...
- 修复实时任务页面切换状态 Tab 时可能残留上一页脏数据的问题。
- 实时明细请求携带当前任务身份，后端返回明细归属信息并标记过期响应。
- 前端仅渲染匹配当前任务、状态和分页的数据，兼容旧格式分页响应。

### v0.0.16

- 恢复实时任务页面 3 秒 HTTP 轮询，任务快照和当前明细页会定期刷新。
- 明细轮询只刷新当前状态、页码和页大小，避免 Tab 或分页被后台刷新重置。
- 后台刷新复用静默请求隔离，慢请求不被下一轮轮询取消，刷新结果直接替换当前页以避免残留脏数据。

### v0.0.15

- 重写实时任务页面详细任务切换逻辑，移除 SSE 与自动刷新轮询。
- 明细列表改为按任务、状态、分页和页大小变化加载，保留手动重试。
- 精简实时任务相关前端辅助代码，降低切换任务时的请求竞争。
